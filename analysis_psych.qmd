...
This section details the psychometric evaluation of the **Modified Attitudes on Psychedelics Questionnaire (MAPQ)**. Classical test theory (CTT) and latent variable modeling are employed to assess the instrument's reliability and construct validity.
...
To validate the robustness of the factor structure, a **Monte Carlo Simulation** is performed using `mvrnorm`. A dataset is simulated ($N=500$) preserving the original means and covariance structure, but rounded to the discrete Likert scale (1-5).
...
The EFA and CFA are replicated on the simulated dataset to confirm structural stability.
...
Confirmatory Factor Analysis defines the theoretical model where each latent construct (LC) loads onto its respective items.
